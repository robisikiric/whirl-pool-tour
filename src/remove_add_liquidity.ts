...
import { get_SOL_balance, get_USDC_balance, jitoBundleSend, jitoSend, manualSend } from "./utils";
...
export const move_liquidity_between_positions = async (cur_pos: PublicKey, next_pos: PublicKey) => {
  try {
    ...
    if (input_SOL_amount.lt(add_quote.tokenEstA)) { // when SOL balance is insufficient
      ...
      add_liquidity_tx.addInstruction(swapInstruction);
    }
    ...
    add_liquidity_tx.prependInstruction(CU_instruction);
    add_liquidity_tx.addInstruction(JITO_ix);

    // Prepend CU instruction
    decrease_liquidity_tx.prependInstruction(CU_instruction);
    add_liquidity_tx.addInstruction(JITO_ix);
    ...
    // manual build
    const built1 = await decrease_liquidity_tx.build({
      maxSupportedTransactionVersion: 0,
      lookupTableAccounts: [lookupTableAccount],
    });
    const built2 = await add_liquidity_tx.build({
      maxSupportedTransactionVersion: 0,
      lookupTableAccounts: [lookupTableAccount],
    });

    await jitoBundleSend([built1, built2]);
